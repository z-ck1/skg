...
break1 = datetime(2024, 10, 1)  # Start of middle period
break2 = datetime(2025, 6, 1)   # Start of final 
...
tick_dates = [
    ...
    datetime(2024, 10, 1),  # Segment boundary
    datetime(2025, 1, 1),   # Additional tick
    datetime(2025, 3, 1),   # Additional tick
    datetime(2025, 6, 1),   # Segment boundary
...
tick_labels = [
    ...
    '2024-10-01',           # 2024-10-01 break
    '2025-01-01',           # 2025-01-01 new year
    '03-01',                # 2025-03-01 2 month
    '2025-06-01',           # 2025-06-01 break
...
def transform_x(days):
    ...
    for i, d in enumerate(days):
        if d < break1_days:
            transformed[i] = (d / break1_days) * (1/3)
        elif d < break2_days:
            transformed[i] = (1/3) + ((d - break1_days) / (break2_days - break1_days)) * (1/3)
        else:
            transformed[i] = (2/3) + ((d - break2_days) / (end_days - break2_days)) * (1/3)
    return transformed
...
ax2.legend(loc='lower left')  # Move legend to bottom left corner
...
for ax in [ax1, ax2]:
    ax.axvline(1/3, color='#AAAAAA', linestyle='--', alpha=0.75)
    ax.axvline(2/3, color='#AAAAAA', linestyle='--', alpha=0.75)
